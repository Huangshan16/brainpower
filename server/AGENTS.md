...
config.ts: dotenv 配置边界，集中提供端口、SQLite 数据库路径与群聊轮次/轮间隔配置
db/: SQLite 数据库模块，承载连接、迁移与 DDL 边界
db/connection.ts: better-sqlite3 连接边界，开启 foreign_keys 并返回同步 SQLite 句柄
db/migrate.ts: SQLite schema 初始化器，读取 schema.sql、归一化 legacy persona origin_ref，并按需重建 participants/messages/runs 约束
db/schema.sql: Digital Mentor Matrix 核心 SQLite DDL，定义 people/source/fragment/skill/evaluation/critique/job/conversation/message/run 全部持久化边界
...
routes/conversationRoutes.ts: 会话路由，提供 create/list conversation、participant、message、run 启停与单 run 查询入口
...
services/conversationRunService.ts: 会话运行服务，封装 direct/group run 的回复生成、多轮异步群聊编排、状态迁移与 stop 行为
...
test/app.test.ts: Express 应用路由测试，验证 health、people、persona import 与 conversation group run 的 HTTP 契约
test/conversationRunService.test.ts: 会话运行服务测试，验证 direct/group run、多轮完成、stop 与消息保留语义
test/conversationService.test.ts: 会话服务测试，验证 conversation、participant、message 的最小持久化闭环
...
test/db.test.ts: 数据库迁移回归测试，验证核心表创建、legacy people 升级与 origin_ref 归一化去重
