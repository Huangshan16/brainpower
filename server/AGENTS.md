...
db/migrate.ts: SQLite schema 初始化器，读取 schema.sql、归一化 legacy persona origin_ref 并补齐唯一索引/参与者约束
db/schema.sql: Digital Mentor Matrix 核心 SQLite DDL，定义 people/source/fragment/skill/evaluation/critique/job 与 conversation 相关表
...
test/app.test.ts: Express 应用路由测试，验证 health、people list、persona import 成功/失败语义与 HTTP 契约
