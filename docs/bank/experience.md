...
- 对话工作台的参与者、消息、run 必须围绕同一 `conversationId` 同步；单聊前自动入场能消除“有人发言但会话里没人”的状态错位。
- 真正的群聊需要后端多轮异步编排与可停止的 run 状态；一次性广播只是批量单聊，前端无法提供可信的“进行中/终止/完成”语义。
- 聊天产品测试至少要同时覆盖服务层 run 状态机、前端 API seam 与浏览器端真实点击录证；只测其中一层，都会把交互假象当成可用性。
- 前端不要伪造 skill 快照 ID；参与者入会话应由后端自动解析最新 skill，没有就允许 `skillId = null`。否则一旦接入真实人物库，`conversation_participants` 的外键就会把“先聊天后蒸馏”这条正常路径炸掉。
- 聊天生成和结构化评审是两种不同契约：评审继续返回 JSON，聊天必须走按 speaker 约束的纯文本回复；否则真实群聊会被引号、fence 或伪 JSON 打断。
- 群聊消息刷新不要只赌在隐式 effect 定时器上；真实浏览器里更稳的做法是显式 run polling，启动时立刻拉取，完成/终止时立刻收口。
