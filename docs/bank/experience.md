...
- 接第三方 README 做导入时，要按真实上游格式写解析测试；上游从 bullet list 漂移到 Markdown 表格后，没有回归测试的解析器通常只会静默导入空列表。
- 前端从 seed 切到真实人物库时，左栏、中栏、右栏必须一起消费同一份异步真相源，并明确设计空库状态；否则人物选择会悬空，工作区控件会失去语义。
- Playwright 浏览器录证不要绕开 strict mode；同名元素一旦冲突，就用“容器 + role + exact”把 locator 收窄，这既能稳住测试，也能暴露界面语义重叠。
- 前后端端口不要写成两份常量；Vite `/api` 代理必须直接跟随后端 `.env` 的 `PORT`，否则页面看起来像“接口挂了”，本质只是代理打错端口。
