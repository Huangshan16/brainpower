/**
 * [INPUT]: 依赖 React 状态副作用、ApiClient seam 与人物库摘要编排对话状态机
 * [OUTPUT]: 对外提供 useConversationController hook
 * [POS]: src/components 的对话状态控制器，被 App 组合进主工作区与右侧栏
...
export function useConversationController({
  ...
  useEffect(() => {
    if (mode === "group" && participants.length < 2 && runState !== "group_running") {
      setMode("direct");
    }
  }, [mode, participants.length, runState]);
  ...
  return {
    ...
    isBusy,
    isComposerDisabled: !api || isBusy || runState === "group_running",
    jobNote,
    messages,
    mode,
    participants,
